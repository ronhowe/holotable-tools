    ...
    foreach ($icon in $Context.front.icons) {
        if (($icon -ne "Defensive Shield") -and ($icon -ne "Maintenance")) {
            ...
            if ($Context.front.type -eq "Defensive Shield") {
                Write-Warning "Excluding Defensive Shield icon from $($Context.front.title)."
            }
            elseif ($Context.front.type -eq "Maintenance") {
                Write-Warning "Excluding Maintenance icon from $($Context.front.title)."
            }
        }
    }
...
function ConvertTo-CdfLine {
    ...
    try {
        ...
        if ($Context.id -eq 6501) {
            Write-Warning "Hard-coding gametext of Luke Skywalker, The Emperor's Prize."
        ...
        if ($Context.id -eq 5959) {
            Write-Warning "Hard-coding gametext of The Falcon, Junkyard Garbage."
